...
function Floater({ position, scale, speed = 1, geometry, variant }: FloaterProps) {
  ...
  const Geo = () => (
    <>
      {geometry === "ico" && <icosahedronGeometry args={[1, variant === "grid" ? 1 : 4]} />}
      {geometry === "torus" && <torusKnotGeometry args={[0.7, 0.22, 160, 24]} />}
      {geometry === "octa" && <octahedronGeometry args={[1, variant === "grid" ? 0 : 3]} />}
...
export default function Scene3D() {
  // stable layout — no runtime randomness
